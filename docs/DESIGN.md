...
Context Bridge is a local context provider for LLMs. It aggregates data from disparate sources (filesystem, GitHub, SSH, Obsidian) and serves it to MCP-compatible clients. The goal is to give LLMs the same situational awareness a human developer has when sitting at their machine.
...
1. A developer can install Context Bridge, point it at 2 repos and 1 Obsidian vault, and query them from Claude Desktop within 10 minutes.
2. File changes are reflected in context within 5 seconds.
3. SSH command output can be retrieved and referenced by the LLM in a single conversation turn.
